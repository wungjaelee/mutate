...
(module+ test
  ...
  ; different types of test result
  (check-equal? 3 3) ; pass
  (check-equal? 3 4) ; fail
  (check-equal? (get-length 3) 4) ; fail if get-length is not mutated, error if get-length's then else is swapped
